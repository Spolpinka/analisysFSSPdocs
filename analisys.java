...
//метод для анализа каждого переданного файла
    public static ArrayList <String> countDocs (String filename) {
            ...
            if (file.isDirectory()){
                ...
                for (int i = 0; i < pathNames.length; i++) {
                    fullNamesForSubdir.add(pathNames[i].getPath());
                }
                try{
                    for (int i = 0; i < fullNamesForSubdir.size(); i++) {
                        content += Files.lines(Paths.get(fullNamesForSubdir.get(i))).reduce("", String::concat);
                    }
                } catch (IOException e) {
                    e.printStackTrace();
